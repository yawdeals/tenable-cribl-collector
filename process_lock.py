...
class ProcessLock:
    ...
    def __init__(self, lock_file='tenable_collector.lock', lock_dir='locks', timeout=600):
        """
    ...
    def acquire(self):
        ...
        if os.path.exists(self.lock_file):
            ...
            if lock_age < self.timeout:
                ...
                self.logger.warning("Removing stale lock file (Age: {0:.0f}s)".format(lock_age))
                try:
                    os.remove(self.lock_file)
                except Exception as e:
                    self.logger.error("Failed to remove stale lock: {0}".format(e))
                    return False
        ...
        try:
            ...
            self.logger.info("Process lock acquired (PID: {0})".format(os.getpid()))
            return True
        except Exception as e:
    ...
    def __enter__(self):
        ...
        if not self.acquire():
            raise RuntimeError("Unable to acquire process lock - another instance is running")
        return self
