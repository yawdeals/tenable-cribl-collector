...
class FileCheckpoint:
    ...
    def _load_checkpoint(self, key):
        ...
        try:
            ...
            logging.error("Error loading checkpoint {}: {}".format(filepath, e))
        ...
        for k in keys_to_flush:
            ...
            try:
                ...
                if len(id_tracking) > self.max_ids:
                    sorted_ids = sorted(id_tracking.items(), key=lambda x: x[1], reverse=True)
                    id_tracking = dict(sorted_ids[:self.max_ids])
                    logging.warning("Checkpoint {} trimmed to {} IDs".format(k, self.max_ids))
                ...
                logging.debug("Flushed checkpoint {}: {} IDs".format(k, len(id_tracking)))
            except Exception as e:
    ...
    def add_processed_id(self, key, item_id):
        ...
        current_time = int(time.time())
        self._cache[key].setdefault('id_tracking', {})[str(item_id)] = current_time
        self._dirty_keys.add(key)
    ...
    def add_processed_ids_batch(self, key, item_ids):
        ...
        self._dirty_keys.add(key)
        self._pending_count[key] = self._pending_count.get(key, 0) + len(item_ids)
    ...
    def get_checkpoint_stats(self, key):
        ...
        try:
            ...
            logging.error("Error getting checkpoint stats for {}: {}".format(key, e))
    ...
    def cleanup_all_checkpoints(self):
        ...
        for key in self.get_all_checkpoints():
            try:
                ...
                logging.info("Cleaned checkpoint {}: {} active IDs".format(key, len(processed_ids)))
            except Exception as e:
                logging.error("Error cleaning checkpoint {}: {}".format(key, e))
