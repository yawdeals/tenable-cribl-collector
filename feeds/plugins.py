...
class PluginFeedProcessor(BaseFeedProcessor):
    ...
    def __init__(self, tenable_client, checkpoint_mgr, hec_handler, batch_size=5000):
        super(PluginFeedProcessor, self).__init__(
            ...
            "Plugin Metadata", "tenableio_plugin", "tenable:io:plugin", batch_size)
    ...
    def process(self):
        ...
        try:
            ...
            for family in families:
                ...
                self.logger.info("Processing plugin family: {0}".format(family_name))
                
                try:
                    family_details = self.tenable.plugins.family_details(family_id)
                    ...
                    for plugin_summary in plugins:
                        ...
                        try:
                            plugin_details = self.tenable.plugins.plugin_details(plugin_id)
                            plugin_details['family_name'] = family_name
                            plugin_details['family_id'] = family_id
                            
                            if self.send_event(plugin_details, item_id=str(plugin_id)):
                                event_count += 1
                                self.log_progress(event_count)
                        except Exception as e:
                            self.logger.warning("Failed to fetch details for plugin {0}: {1}".format(plugin_id, str(e)))
                            continue
                except Exception as e:
                    self.logger.warning("Failed to process family {0}: {1}".format(family_name, str(e)))
                    continue
            
            self.flush_events()
            self.log_completion(event_count)
        except Exception as e:
            self.logger.error("Error processing plugin feed: {0}".format(str(e)))
...
class ComplianceFeedProcessor(BaseFeedProcessor):
    ...
    def __init__(self, tenable_client, checkpoint_mgr, hec_handler, batch_size=5000):
        super(ComplianceFeedProcessor, self).__init__(
            tenable_client, checkpoint_mgr, hec_handler,
            "Compliance Findings", "tenableio_compliance", "tenable:io:compliance", batch_size)
    ...
    def process(self):
        ...
        try:
            ...
            for scan in scan_list:
                ...
                self.logger.info("Processing compliance findings from scan: {0}".format(scan_name))
                        ...
                        try:
                            ...
                            for compliance in compliance_items:
                                compliance_key = "{0}_{1}_{2}".format(
                                    scan_id, host_id, compliance.get('plugin_id', 'unknown')
                                )
                                ...
                                if self.send_event(compliance_event, item_id=compliance_key):
                                    event_count += 1
                                    self.log_progress(event_count)
                        except Exception as e:
                            self.logger.warning("Failed to get host details for {0}: {1}".format(hostname, str(e)))
                            continue
                    
                    if scan_timestamp > last_timestamp:
                        self.set_last_timestamp(scan_timestamp)
                except Exception as e:
                    self.logger.warning("Failed to process scan {0}: {1}".format(scan_name, str(e)))
                    continue
            
            self.flush_events()
            self.log_completion(event_count)
        except Exception as e:
            self.logger.error("Error processing compliance feed: {0}".format(str(e)))
